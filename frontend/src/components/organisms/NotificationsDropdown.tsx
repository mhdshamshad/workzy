...
export function NotificationsDropdown({ role = ROLE.USER }: { role?: Role }) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        align="end"
        className="w-105 p-0 shadow-2xl rounded-2xl border-muted/50"
        sideOffset={8}
      >
        <div className="flex flex-col bg-background/95 backdrop-blur-xl rounded-2xl overflow-hidden">
          <div className="px-5 py-2 flex flex-row items-center justify-between border-b border-border/50 bg-linear-to-r from-primary/5 to-transparent">
            <div>
          ...
          <div className="flex items-center gap-2 px-5 py-3 border-b border-border/50 bg-muted/20">
            ...
            >
              ...
              {unreadCount > 0 && (
                <Badge
                  variant="secondary"
                  className={`ml-1.5 px-1.5 min-w-5 h-5 flex items-center justify-center ${
                    filter === 'unread'
          ...
          {isLoading && notifications.length === 0 ? (
            ...
            <div className="max-h-100 overflow-y-auto p-3 space-y-2 custom-scrollbar">
              {notifications.map(n => (
                <div
                  key={n.id}
                  className={`group relative flex items-start gap-4 p-4 rounded-2xl transition-all duration-200 cursor-default ${
                    !n.read
                      ? 'bg-primary/4 hover:bg-primary/8 shadow-sm'
