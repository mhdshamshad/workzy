        ...
        {isLoading ? (
          <div className="flex gap-4">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="basis-1/2 md:basis-1/3 lg:basis-1/4 xl:basis-1/5 shrink-0">
          ...
          <AppCarousel
            ...
            badge={
              <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-white dark:bg-card rounded-full text-indigo-600 dark:text-indigo-400 text-xs font-medium shadow-xs">
                <TrendingUp className="w-3.5 h-3.5" />
                Most Popular
              </div>
            }
            title={title || 'Top Booked Services'}
            subTitle={subTitle || 'Join thousands of satisfied customers'}
          />
        )}
      </div>
    </section>
